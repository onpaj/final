...
export default function GroupDetail({ group, groupSlug, onCategoryClick, onBack }: Props) {
  ...
  return (
    <div>
      ...
      <h2 className="text-xl font-bold mb-4">{groupSlug ? t("cat." + groupSlug, { defaultValue: group.name }) : group.name}</h2>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {group.categories.map((c) => (
              ...
              >
                <td className="px-4 py-3">{c.category_slug ? t("cat." + c.category_slug, { defaultValue: c.name }) : c.name}</td>
